#!/usr/bin/env python3
"""Training curves, Director vs SOM-line+LiP, one square panel per environment.

Same row layout and styling as the goal-geometry figures so the two sit
together in the paper: boxed axes, light gridlines on both, outward ticks,
plain titles, a thick mean line and a +/-1 std band across the four seeds.

Reads metrics.jsonl straight from the run dirs on /work -- the arm_comparison
JSON only covers the two tasks the first batch ran.

  python3 make_figure_curves_row.py --out arm_curves_row
"""
import argparse
import glob
import json
...
# (task, short title, director exp numbers, som+lip exp numbers)
TASKS = [
    ('dmc_cartpole_swingup', 'Cartpole', range(502, 506), range(534, 538)),
    ('dmc_hopper_stand', 'Hopper Stand', range(506, 510), range(538, 542)),
    ('dmc_cartpole_swingup_sparse', 'Cartpole Sparse', range(550, 554),
     range(558, 562)),
...
def build(ymaxes):
  ...
  for k, (task, title, dexp, mexp) in enumerate(TASKS):
    x0 = PL + k * (P + GAP)
    ymax = ymaxes[k]
    ...
    for (label, color), exps in zip(ARMS, (dexp, mexp)):
...
def main():
  ...
  ap.add_argument('--out', default=str(HERE / 'arm_curves_row'))
  a = ap.parse_args()

  ymaxes = []
  for task, title, dexp, mexp in TASKS:
    hi, ns = 0.0, []
    for exps in (dexp, mexp):
    ...
    print('%-30s n_dir=%d n_arm=%d  ymax=%.0f'
          % (task, ns[0], ns[1], ymaxes[-1]))
