...
| e161 | 4658046 | D combined | hopper | BIG (short-a100) | 0.0 | ❌ CANCELLED 07-13 @0.73M (resumed as 4660124, then cancelled: rewrate 1.3e-4 dead; struct ablation moot on an all-dead task) |
...
| e181 | 4660141 | plain_vark tgt8 + agg=sum | hopper | e178 + mgr_reward_agg mean→sum | ❌ CANCELLED @0.62M (rewrate 3e-4, dead; e178 pulses without sum) |
| e182 | 4660142 | D-fix tgt8 | hopper | e173 + DUR_TARGET 4→8 | ❌ CANCELLED @0.60M (rewrate 0, dead; longer holds do not rescue the masked recipe) |
| e183 | 4660143 | D-fix + mgr_expl_weight 1.0 | hopper | e173 + 10× manager exploration | ❌ CANCELLED @0.60M (rewrate 1.5e-3 flat; 10× exploration does not rescue) |
| e184 | 4660144 | D-fix tgt8 | acrobot | e177 + DUR_TARGET 4→8 | ❌ CANCELLED @0.59M (rewrate 5e-4 vs e180 baseline 0.125 — masked recipe dead on acrobot) |
...
### e185–e188 · HiTS-style timed subgoals: countdown-conditioned worker (launched 2026-07-13)

**Motivation.** In every var-K run the worker receives ONLY ``concat([deter, stoch,
goal])`` — no information about how long it has to reach the goal. Its λ-return is cut
at goal switches (Director ``split_traj``), so under variable K it is trained to predict
a return over a window whose end it cannot observe: random horizon (1–16, mean 4) =
pure variance injected into the critic, and ~half of e124's reach-time per goal.
Observed: worker goal-reward is LOWEST at the peak of the collapsing plain-var-K run
(e167: 0.14–0.23 vs e46's 0.26→0.46 climb, e171's 0.32–0.43). HiTS (Guertler et al.,
NeurIPS 2021, arXiv:2112.03100; graph_rl repo) solves exactly this: the manager emits a
*timed* subgoal (g, Δt); the worker observes the **countdown** (decremented each step,
normalized to [-1,1] by Δt_max), its value treats the deadline as terminal, and fixing
Δt removes the SMDP non-stationarity on the manager side. Key mapping insight: **our
var-K manager already implements the manager half of timed subgoals** (duration fixed
in advance, no early interruption); what was missing is the worker half.

**Implementation (``agent.worker_timed_goals``, default False = old behavior).** The
normalized countdown (steps left incl. current, /budget → [-1,1]; budget =
``goal_duration_max`` under var-K, K under fixed-K) is appended to the worker policy
AND value inputs at all sites: imagination rollout (scan carries it; new per-step
output), both imagination-loss paths (windowed fixed-K: K..0 within window; dense
var-K: stacked countdowns), replay worker value (``_manager_skills_on_sequence`` now
also outputs ``countdown``), real acting (from the ``mgr_step`` carry), and the
bootstrap last-action. Report/viz paths default to full budget. NOT ported from HiTS
(replay-relabeling machinery incompatible with imagination training): hindsight action
relabeling, HER with time relabeling, deadline-only sparse reward, testing transitions.
Worker reward stays dense cosine — the test isolates *horizon observability*.
Smokes: 4660289 (var-K + flag), 4660290 (flag off, regression), 4660291 (fixed-K +
flag) — all COMPLETED 0:0.

| Exp | Job | Recipe + WTG | Task | Scale | Compare against |
|---|---|---|---|---|---|
| e185 | 4660298 | plain_vark tgt8 + countdown | hopper | BIG | e178 (same minus countdown; pulse 3e-3 @0.6M) |
| e186 | 4660299 | D-fix tgt4 + countdown | cartpole | small | e170 (668 / peak 786) |
| e187 | 4660300 | plain_vark tgt4 + countdown | cartpole | small | e166 (130 / peak 192) |
| e188 | 4660301 | mask_fixedk K8 + countdown | cartpole | small | e162 (272 / peak 575) |

**Hypotheses.** (1) e187 is the sharpest worker-confusion test: plain var-K tgt4 is the
recipe where the invisible random deadline hurts most (short + random windows, no
struct anchor); if the countdown lifts it well above 130 (toward e46-tier), worker
horizon-observability is a real bottleneck. (2) e186 vs e170: does the countdown help
even the best recipe (expect wkr_goal_rew ↑; score may or may not). (3) e188 vs e162:
fixed-K phase-observability test (window stats are stationary at K=8, so expect a
smaller effect than var-K). (4) e185 vs e178: does countdown + tgt8 turn hopper's weak
pulse into learning. Watch ``wkr_goal_rew`` (primary mediator) alongside score.

---

### Interim triage @0.6M + e189–e195 · struct×duration×countdown factorial (2026-07-13)

**Triage of e178–e184 at ~0.6M** (e124 reference: unambiguously alive by this step
count): e181 (sum agg), e182 (D-fix tgt8 hop), e183 (D-fix 10× expl), e184 (D-fix tgt8
acrobot) all dead → CANCELLED to free 4 A100s (see table above). Kept: e178 (hopper
pulse, rewrate 2.9e-3 and rising — 30× the dead floor but below the 1e-2 alive bar),
e179 (cartpole tgt8 53 @0.6M, stability question resolves later), e180 (Director
acrobot LEARNING, rewrate 0.125, ~46 — base hierarchy handles acrobot; our recipes are
the problem there), e185 (WTG flagship, pending). e161 (struct-0 hopper, short-a100)
also cancelled — hopper is dead with AND without struct in the D recipe, so that
ablation is uninformative; e160 (struct-0 cartpole) continues.

**Struct is decoupled from masking** (answering "does struct only help with masking?"):
``goal_struct_weight`` shapes the goal-AE code geometry (code distance ∝ state
distance) regardless of masks. Evidence it matters without masks: e46 (plain var-K +
struct200 tgt8) 724 vs e166/e167 (struct 0) 130/97-collapse; and the e167 collapse
anatomy (manager advantage dies exactly when worker goal-reward drops during
unanchored AE drift) points at struct-as-code-stabilizer, not just locality. BUT no
sparse-task cell has ever run var-K + struct WITHOUT masks — struct could stabilize
codes (helping) or enforce proposal locality (killing exploration). e189 settles it.

**Struct as a Lagrangian?** Conceptually sound: constrain "struct loss ≤ tol" via the
same dual ascent as mask/duration, self-relaxing when geometry is already consistent
and auto-stiffening during AE-drift phases (exactly when e167-style collapses happen),
removing the hand-tuned w=200. The existing (never-run) ``goal_struct_adapt`` AutoAdapt
machinery approximates this (multiplier on struct loss toward target 0.01). Piloted as
e195 after smoke 4660314; a symmetric corr-targeted version can follow if the pilot
behaves.

| Exp | Job | Config | Task | Scale | Question |
|---|---|---|---|---|---|
| e189 | 4660308 | plain_vark tgt8 + struct200 | hopper | BIG | does struct kill e178's pulse (locality) or strengthen it (stabilizer)? |
| e190 | 4660309 | pure Director (e123 script) | cartpole | BIG | missing Director-matched cartpole control |
| e191 | 4660310 | pure Director (e123 script) | cheetah | BIG | cheetah control redo — e123 stopped at 2.6M (peak 425) |
| e192 | 4660311 | plain_vark tgt8 + countdown | acrobot | BIG | best-guess sparse recipe on acrobot vs e180 baseline |
| e193 | 4660312 | plain_vark tgt4 + struct200 | cartpole | small | completes e46/e166 2×2: struct at SHORT target |
| e194 | 4660313 | plain_vark tgt8, struct 0 | cartpole | small | completes 2×2: long target without struct (small mirror of e179) |
| e195 | 4660317 | plain_vark tgt4 + goal_struct_adapt (init 200) | cartpole | small | struct-Lagrangian pilot on the cell that collapses without struct (smoke 4660314 ✅) |

Small-cartpole 2×2 (with e46 tgt8+struct=724 and e166 tgt4+nostruct=130) disentangles
the duration-target and struct confound behind e46-vs-e166. GPU budget after this
batch: A100 8/8 (e178/179/180/185/189/190/191/192), V100 6/8 (e186/187/188/193/194 +
e195 pending smoke), short-a100 e160.

---

### Pre-registered hypotheses & readout logic — 2026-07-13 board (e160, e178–e180, e185–e195)

Written while all 15 cells are in flight, before results. Reference values used
throughout: dead-hopper floor ``epstats/reward_rate`` ≈ 1e-4; alive bar = 1e-2 by 1M
(e124 was unambiguous by then, ~300 by 1.5M); healthy worker goal-reward 0.32–0.46
(e171/e46), collapse-zone 0.14–0.23 (e167); healthy manager advantage ~0.01–0.02
sustained (e171), collapse = spike then |adv| < 0.01 permanently (e167). Primary
mediators to read BEFORE score: ``wkr_goal_rew`` (countdown cells), ``goal/struct_corr``
+ ``goal/rec_mean`` (struct cells), ``mgr_extr_adv`` (collapse), ``goal/mask_frac`` ×
``mgr_duration`` → blk/step (cost claims).

**e178 — plain var-K, tgt8, struct0, hopper BIG (hold-length main test).**
H: short commitment (target 4) was the primary var-K hopper killer; 8-step holds
restore enough exploration depth to find reward. Expected if fully true: rewrate
crosses 1e-2 by ~1–1.5M, score follows e124's shape at some lag (≥100 by 2.5M).
Status @0.6M: pulse 2.9e-3, rising — between hypotheses.
→ If alive & scores: duration target was the main killer. Fix = per-task duration
  targets (or duration floors ≥8 on sparse tasks); var-K machinery exonerated; the
  program's tgt4 default (cartpole-tuned) is retired off-cartpole.
→ If pulse plateaus at 1e-3–1e-2 without score takeoff: commitment is necessary but
  not sufficient — the residual is worker horizon-observability (read e185) or
  duration-head REINFORCE variance.
→ If pulse dies back to 1e-4: the pulse was exploration noise; var-K machinery itself
  (not its target) is indicted, and e185/e189 become the only hopper hopes.

**e179 — plain var-K, tgt8, struct0, cartpole BIG (stability sentinel).**
H: e167's rise-then-collapse (peak 658 @~1.6M → ~0 by 2M) was caused by unanchored
goal-code drift, not by target 4; tgt8 alone does NOT prevent it (struct does).
Expected: learns early (@0.6M: 53 ✓ vs e167's 33), then EITHER collapses after ~1.5M
(supports H) or stays stable (refutes H for tgt8).
→ If it collapses like e167 (advantage → 0 + wkr_goal_rew dip while rec_mean climbs):
  struct (or another code anchor) is REQUIRED for var-K stability on BIG regardless of
  target → all future var-K runs carry struct or struct-adapt; e46's 724 is attributed
  mainly to struct, and the e46-vs-e166 confound resolves toward struct.
→ If stable ≥600 at 4M: tgt8 suffices without struct on dense tasks at BIG scale →
  the collapse was a short-target phenomenon; struct demoted to optional.
→ If flat/low (~100–300) without a collapse event: tgt8 itself costs cartpole score
  (consistent with e58 masked tgt8 511 < tgt4) → duration target is a genuine
  per-task tradeoff, strengthening the case for adaptive duration.

**e180 — pure Director, acrobot BIG (task-difficulty control).**
H: acrobot is learnable by the unrestricted base hierarchy. Status @0.6M: rewrate
0.125, score ~46 climbing — H already supported. Expected plateau: 150–400 by 4M
(between hopper ~300 and the impulsive-task difficulty; no strong prior).
→ Whatever it plateaus at becomes the acrobot reference: every restricted-recipe
  acrobot cell is judged against it, and "our restrictions break sparse tasks, the
  base does not" is confirmed on a second task (already true directionally).
→ If it inexplicably dies late: acrobot marginal even for the base — e192's readout
  weakens to "no worse than baseline".

**e185 — e178 + worker countdown, hopper BIG (horizon observability × hold length).**
H: the worker's unobservable random deadline is a second, independent killer; making
it observable turns e178's pulse into learning. Expected if true: wkr_goal_rew
persistently above e178's at matched steps (this is the mediator — check it FIRST),
rewrate ≥ 3× e178's at 1M, alive by 1.5M.
→ If e185 ≫ e178: worker_timed_goals becomes default in all var-K runs; combined with
  e178's branch this defines the sparse-task recipe (tgt8 + countdown ± struct per
  e189). HiTS's mechanism transfers to imagination-trained HRL.
→ If e185 ≈ e178 (both weak): horizon observability is not binding on hopper —
  the deficit is goal-proposal quality/exploration, not worker execution; pivot to
  manager-side interventions (non-local proposals, worker task-reward mixing).
→ If e185 < e178: the extra input hurt (critic distraction / policy overfit to
  deadline) — would show as wkr_goal_rew ↑ but score ↓; unlikely, but if seen,
  try duration-at-switch conditioning instead of per-step countdown.

**e186 — combined recipe + countdown, cartpole small (vs e170: 668, peak 786).**
H: countdown raises worker reliability even inside the best recipe. Expected:
wkr_goal_rew 0.45+ (vs e170-era ~0.4), score ≥ e170; a null score result with
wkr_goal_rew ↑ still supports adopting it (cheaper coordination).
→ If score ↑ meaningfully (≥ +50 on last10): countdown is a general upgrade, not a
  sparse-task patch → default-on everywhere.
→ If wkr_goal_rew unchanged: the countdown input is being ignored under masking
  (masked edits keep goals near → deadlines rarely bind) — restrict the flag to
  plain var-K contexts.

**e187 — plain var-K tgt4 + countdown, cartpole small (vs e166: 130, peak 192).**
Sharpest single worker-confusion test: short + random windows, no struct, where the
invisible deadline should hurt most. H: a large share of e166's weakness is worker
confusion. Expected if true: ≥300 by 2M (e166 never passed 192); wkr_goal_rew climbing
past 0.5 (e166 held ~0.46 — note its worker was already decent, so H may instead fail
here and implicate the MANAGER side at small scale).
→ If big lift: worker confusion confirmed as a primary var-K cost → countdown default;
  reread e166/e167 history as worker-side failure.
→ If flat ≈130: worker confusion NOT binding at small scale (consistent with e166's
  healthy 0.46 worker reward) → e167's BIG-scale collapse was manager/AE-side (drift),
  raising e193/e195 (struct cells) to primary on the cartpole question.
→ Late collapse either way is expected without struct; judge e187 on its pre-2M slope,
  not its tail.

**e188 — mask fixed-K8 + countdown, cartpole small (vs e162: 272, peak 575).**
H: at fixed K the window statistics are stationary, so phase-observability adds little
(the worker has implicitly calibrated to 8-step projects). Expected: small effect
(±50 on last10).
→ If large lift (≥400 sustained): even FIXED-K Director leaves performance on the
  table by hiding the phase — a standalone, publishable observation; countdown
  default-on for fixed-K too.
→ If null: consistent with H; countdown's value is specific to variable durations.

**e189 — plain var-K tgt8 + struct200, hopper BIG (struct's two faces, decisive).**
Struct simultaneously (a) stabilizes code meaning under AE drift and (b) localizes
proposals (code distance ∝ state distance). On dense tasks (a) dominates (e46).
H (from the locality post-mortem): on hopper (b) dominates — struct will DAMP e178's
pulse. Counter-H (from the e167 collapse anatomy): (a) dominates — struct anchors
codes and AMPLIFIES the pulse.
→ If rewrate falls toward 1e-4 (pulse killed) while struct_corr ≈0.97: locality
  confirmed as the sparse-task mechanism → struct must be gated/scheduled (anneal in
  after first reward, cap corr, or adaptive-relaxing e195-style), and the combined
  recipe's hopper failure is mechanistically explained (masks + struct both localize).
→ If rewrate ≥ e178's (pulse survives/strengthens), especially if it goes alive:
  struct is primarily a stabilizer → include in every var-K run incl. sparse; the
  locality story is then carried by MASKS alone (consistent with masked cells dying
  even at fixed K with high worker entropy).
→ If unchanged: struct orthogonal on hopper; both faces weak; attribution stays with
  duration/countdown axes.

**e190 / e191 — pure Director, cartpole / cheetah BIG (Director-matched controls).**
H: the base reproduces Director-tier dense-task scores at our scale: cartpole ~800+,
cheetah ~450–550 at 4M (e123 reached peak 425 by 2.6M before timing out).
These two cells DEFINE the denominators for the paper's headline claim.
→ If e190 ≈ 800 and combined recipe (e171: 776/844) matches it at blk/step ≈0.6–1.1:
  "sparser decisions at no dense-task cost" is supported on cartpole.
→ If e190 ≥ 900: the combined recipe has a real dense-task deficit — report it.
→ If e191 lands ~500 vs e175's 465/508: combined ≈ baseline on cheetah (strong,
  quotable); if e191 ≫ 550, the Lagrangian recipe still lags the base on cheetah.
→ If either control badly underperforms published Director: our base has a
  reproduction gap and ALL recipe-vs-base comparisons inherit that caveat.

**e192 — plain var-K tgt8 + countdown, acrobot BIG (transfer of best-guess recipe).**
H: whatever combination rescues hopper also rescues acrobot (shared sparse-task
mechanism). Expected if true: rewrate well above 1e-3 by 1M and score tracking some
lag of e180.
→ If alive and roughly tracks e180: the sparse-task fix generalizes → revised recipe
  is (var-K tgt8 + countdown ± struct per e189), ready for 4-task seed-replicated
  evaluation.
→ If dead while e180 climbs and hopper cells (e178/e185) pulse: acrobot has an
  ADDITIONAL failure mode beyond hopper's (candidate: impulsive timing needs even
  longer commitment, or duration-head entropy on this task) — split the tasks in the
  analysis.

**e193 / e194 — plain var-K cartpole small: tgt4+struct200 / tgt8+struct0 (2×2).**
Completes {tgt4, tgt8} × {struct 0, 200} with e46 (tgt8+struct = 724) and e166
(tgt4+none = 130). H (struct-primary): e193 ≈ 600–750, e194 ≤ 300. Counter-H
(duration-primary): e193 ≤ 300, e194 ≈ 600+.
→ e193 high, e194 low → struct is THE load-bearing var-K ingredient on dense tasks;
  e46's 724 attributed to struct; duration target secondary.
→ e193 low, e194 high → duration target is load-bearing; struct secondary on dense.
→ Both high → either suffices (redundant stabilizers); cheapest one wins (drop struct
  where it risks locality).
→ Both low → superadditive interaction: need BOTH (e46 = the unique working corner);
  the recipe is fragile and adaptive versions (e195, duration-lagrangian) matter more.
Also a scale check: e194 vs e179 (same recipe, small vs BIG) — if e194 is stable while
e179 collapses, BIG-scale worker weakness (e167's 0.15 wkr_goal_rew vs e166's 0.46) is
part of the collapse story.

**e195 — plain var-K tgt4 + goal_struct_adapt (init 200 → target loss 0.01), small.**
H: an adaptive multiplier can replace the hand-tuned w=200: relax when code geometry
is consistent, stiffen under drift. Expected: adapt scale settles WELL below max
(watch ``goal/struct_adapt_scale``), struct_corr ≥0.9 maintained, score ≥ e193's.
→ If tracks/beats e193 with a settling multiplier: adopt adaptive struct as default
  (removes per-task w tuning) and promote to the symmetric corr-target Lagrangian
  (mirroring the duration controller) as a code change.
→ If multiplier rails at max 2000: loss-target 0.01 too tight — retune target, or
  switch to corr-targeted form before judging the idea.
→ If multiplier → ~0 and the run collapses e166-style: target too loose; the
  controller relaxed exactly when it was needed → the corr-floor (inequality
  constraint, only push when corr < c) formulation is the right one, not a loss target.

**e160 — combined recipe, struct0, cartpole BIG (short-a100, slow).**
H: within the FULL recipe (masks + both Lagrangians), struct is redundant on dense
tasks — masks already keep proposals local and the running goal anchors meaning.
Expected under H: eventually ≈ e171 (776/844), just late (only ~0.65M so far, requeue
cadence). Counter-evidence would be an e167-style collapse or a low plateau.
→ If ≈ e171: struct droppable from the dense-task combined recipe → simpler recipe,
  and struct's role isolated to plain var-K (per e193) and possibly sparse locality.
→ If ≪ e171 or collapses: struct load-bearing even under masks → it stays, and e189's
  outcome decides whether sparse tasks need a gated version.

**Global decision rule (unchanged):** the first sparse-task cell that goes alive
defines the revised recipe; it is then rerun on all four tasks × ≥2 seeds with the
e190/e191 controls as denominators before any claim enters the paper as a result
rather than an observation. If NOTHING on this board revives hopper/acrobot, the
next interventions are code-level: worker task-reward mixing (small extrinsic share
into the worker objective) and periodic non-local goal proposals (mask-free decisions
every Nth switch), in that order.
